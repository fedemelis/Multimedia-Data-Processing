...
public:
	...
	void increment(const uint8_t& c) {
		++count[c];
		++total;
	}
...
void findBinaryRepresentation(Node* root, uint8_t target, std::vector<int>& path, std::vector<int>& winpath) {
...
int main(int argc, char* argv[]) {
	...
	if (argv[1] == std::string("c"))
	{
		...
		std::vector<uint8_t> caratteri(input_size);
		is.read(reinterpret_cast<char*>(caratteri.data()), input_size);
		...
		{
			leaf[i].data.carattere = static_cast<uint8_t>(i);
	...
	else
	{
		...
		{
			c = 0;
			c = static_cast<uint8_t>(reader(8));
			if (c >= 0 && c <= 255)
			{
				decoding[c] = c;
		...
		for (size_t i = 0; i < numsymbol; i++)
		{
			...
			auto res2 = std::find(dec.begin(), dec.end(), cc);
			auto res = std::find(dec.begin(), dec.end(), cc);
			distance = static_cast<uint8_t>(std::distance(dec.begin(), res));

			while ((res == dec.end()) || (dimensione != length[distance]))
			{
				cc <<= 1;
				cc |= reader(1);
				++dimensione;
				...
				distance = static_cast<uint8_t>(std::distance(dec.begin(), res));
				if ((dimensione != length[distance]) && (res != dec.end()))
				{
					res2 = std::find(dec.begin() + distance + 1, dec.end(), cc);
					distance = static_cast<uint8_t>(std::distance(dec.begin(), res2));
					if (dimensione == length[distance])
					{
						beccato = true;
